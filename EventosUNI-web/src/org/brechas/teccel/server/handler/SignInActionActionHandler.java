	...
	public SignInActionResult execute(SignInAction action,
		...
		if (usergae != null) {
			user = ofy().load().type(CurrentUser.class)
					.filter("email", usergae.getEmail()).first().now();
			if (user == null) {
				...
				user.setLogoutUrl((userService.createLogoutURL(action
						.getRequest())));
			...
			user.setLoginUrl(action.getRequest());
			user.setLogoutUrl(action.getRequest());
		}
